...
public sealed partial class StarfallWatchView : Node2D
{
    ...
    )
    {
        ...
        {
            Name = "Player",
            Position = session.InsideStarfallWatch
                ? new Vector2(session.PlayerX, session.PlayerY)
                : CellCenter(new GridPosition(19, 18)),
            ZIndex = 10
        };
        ...
        AddChild(_player);
        AddChild(new StarfallWatchInteractionHints(
            () => _player.CurrentCell,
            () => ResolveNpcTarget(
                _player.TargetCell,
                _player.CurrentCell
            ),
            () => _session.PreviewSelectedTarget(SealRouteTableCell)
        ));
    ...
    public event Action? SealRouteTableRequested;
    ...
    private TargetPreview ResolveTargetPreview()
    {
        ...
        if (IsSealRouteTableArea(target) ||
            IsAdjacent(player, SealRouteTableCell))
        {
            return _session.PreviewSelectedTarget(SealRouteTableCell);
        }
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        else if (IsSealRouteTableArea(target) ||
            IsAdjacent(player, SealRouteTableCell))
        {
            SealRouteTableRequested?.Invoke();
        }
    ...
    private static bool IsSealRouteTableArea(GridPosition cell) =>
        cell.X is >= 12 and <= 27 &&
        cell.Y is >= 5 and <= 11;
...
internal sealed partial class StarfallWatchNpcLayer : Node2D
{
    private readonly GameSession _session;

    public StarfallWatchNpcLayer(GameSession session)
    {
        _session = session;
        ZIndex = 8;
...
internal sealed partial class StarfallWatchInteractionHints : Node2D
{
    private static readonly Rect2 SealRouteTableBounds = new(
        12 * 16,
        5 * 16,
        16 * 16,
        7 * 16
    ...
    private readonly Func<TargetPreview> _tablePreview;
    private double _time;

    public StarfallWatchInteractionHints(
        Func<GridPosition> playerCell,
        Func<VillageNpcState?> npcTarget,
        Func<TargetPreview> tablePreview
    )
    {
    ...
    public override void _Draw()
    {
        ...
        var tablePreview = _tablePreview();
        DrawSealRouteTableHint(
            Distance(player, StarfallWatchView.SealRouteTableCell) <= 4,
            PreviewColor(tablePreview.State)
        );
        DrawCellHint(
            StarfallWatchView.DoorCell,
            Distance(player, StarfallWatchView.DoorCell) <= 3,
            ThemeFactory.Gold
        );
    ...
    private void DrawSealRouteTableHint(bool nearby, Color color)
    {
        var pulse = Pulse(SealRouteTableBounds.GetCenter().X);
        var alpha = nearby ? pulse : pulse * 0.35f;
        DrawRect(
            SealRouteTableBounds.Grow(2),
            new Color(color, alpha),
            false,
            nearby ? 2 : 1
        );
        DrawCircle(
            CellCenter(StarfallWatchView.SealRouteTableCell),
            nearby ? 4 : 2,
