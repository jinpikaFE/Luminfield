...
public sealed partial class TwilightEmporiumView : Node2D
{
    public static readonly GridPosition ManifestCell =
    ...
    )
    {
        ...
        {
            Name = "Player",
            Position = session.InsideTwilightEmporium
        ...
        AddChild(_player);
        AddChild(new TwilightEmporiumInteractionHints(
            () => _player.CurrentCell,
            () => ResolveNpcTarget(
                _player.TargetCell,
                _player.CurrentCell
    ...
    public event Action? ManifestRequested;
    ...
    private TargetPreview ResolveTargetPreview()
    {
        ...
        if (IsManifestArea(target) || IsAdjacent(player, ManifestCell))
        {
            return _session.PreviewSelectedTarget(ManifestCell);
        }
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        else if (IsManifestArea(target) ||
            IsAdjacent(player, ManifestCell))
        {
            ManifestRequested?.Invoke();
        }
...
internal sealed partial class TwilightEmporiumNpcLayer : Node2D
{
    private readonly GameSession _session;

    public TwilightEmporiumNpcLayer(GameSession session)
    {
        _session = session;
        ZIndex = 8;
...
internal sealed partial class TwilightEmporiumInteractionHints : Node2D
{
    private static readonly Rect2 ManifestBounds = new(
        14 * 16,
        4 * 16,
        12 * 16,
        5 * 16
    ...
    public TwilightEmporiumInteractionHints(
    ...
    public override void _Draw()
    {
        ...
        DrawManifestHint(
            Distance(player, TwilightEmporiumView.ManifestCell) <= 4
        );
        DrawCellHint(
            TwilightEmporiumView.DoorCell,
            Distance(player, TwilightEmporiumView.DoorCell) <= 3,
            ThemeFactory.Gold
        );
    ...
    private void DrawManifestHint(bool nearby)
    {
        var pulse = Pulse(ManifestBounds.GetCenter().X);
        var alpha = nearby ? pulse : pulse * 0.35f;
        DrawRect(
            ManifestBounds.Grow(2),
            new Color(ThemeFactory.Mint, alpha),
            false,
            nearby ? 2 : 1
        );
        DrawCircle(
            CellCenter(TwilightEmporiumView.ManifestCell),
            nearby ? 4 : 2,
